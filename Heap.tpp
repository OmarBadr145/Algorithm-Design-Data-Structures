...
public:
    ...
    void insert(T element)
    {
        ...
        /// heapifying the tree going up
        while (index > 0 && tree[this->getParentPosition(index)] > tree[index])
        {
            std::swap(tree[index], tree[this->getParentPosition(index)]); /// swapping index
            /// Update index parents to position after swap
            index = this->getParentPosition(index);
        }
    }
    ...
    void remove(T value)
    {
        ...
        for (auto i = 0u; i < tree.size(); i++)   ///traverse tree to find value we are deleting
        {
            ...
            {
                index = i;   ///set index to the index of value
                break;
            }
        }
